...
import { Users, ArrowLeft, Bell, UserCheck, UserX, MessageSquare } from 'lucide-react';
...
export default function ResidentNotificationsPage() {
  ...
  useEffect(() => {
    notifications.forEach(notification => {
      if (notification.residentId === user?.id && !notification.read) {
        markNotificationAsRead(notification.id);
      }
    });
  }, [notifications, markNotificationAsRead, user]);

  if (!user) {
    return null;
  }

  // Filter and sort notifications for this resident
  const myNotifications = notifications
    .filter(notification => notification.residentId === user.id)
